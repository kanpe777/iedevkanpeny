...
class StructuresController < ApplicationController
  ...
  def create
    @pirty = current_pirty
    pokedb = Pokedb.find(params[:pokedb_id])
    #@structure = Structure.new(params[:structure])
    @structure = @pirty.structures.build(pokedb: pokedb) 

    respond_to do |format|
      if @structure.save
        #format.html { redirect_to @structure, notice: 'Structure was successfully created.' }
        format.html { redirect_to @structure.pirty, notice: 'Structure was successfully created.' }
